...
{
    return A(reg) + signExtend<int16_t, int32_t>(GetNextWord(BUS_NORMAL));
}

uint32_t SCC68070::AddressRegisterIndirectWithIndex8(const uint8_t reg)
{
    const uint16_t bew = GetNextWord(BUS_NORMAL);
...
uint32_t SCC68070::ProgramCounterIndirectWithDisplacement()
{
    ...
    return pc + signExtend<int16_t, int32_t>(GetNextWord(BUS_NORMAL));
}
...
{
    return signExtend<int16_t, int32_t>(GetNextWord(BUS_NORMAL));
}

uint32_t SCC68070::AbsoluteLongAddressing()
{
    const uint16_t high = GetNextWord(BUS_NORMAL);
    const uint16_t low = GetNextWord(BUS_NORMAL);
    return high << 16 | low;
}
