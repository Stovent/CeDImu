...
std::string SCC68070::DisassembleADDI(const uint32_t pc) const
{
    ...
    {
        size = 1;
        data = std::to_string(as<int8_t>(cdi.PeekByte(pc+3)));
    }
    else if(size == 1)
    {
        size = 2;
        data = std::to_string(as<int16_t>(cdi.PeekWord(pc+2)));
    }
    else
    {
        size = 4;
        data = std::to_string(as<int32_t>(cdi.PeekLong(pc+2)));
    }
...
std::string SCC68070::DisassembleANDI(const uint32_t pc) const
{
    ...
    {
        size = 1;
        data = std::to_string(cdi.PeekByte(pc+3));
    }
    else if(size == 1)
    {
        size = 2;
        data = std::to_string(cdi.PeekWord(pc+2));
    }
    else
    {
        size = 4;
        data = std::to_string(cdi.PeekLong(pc+2));
    }
...
std::string SCC68070::DisassembleBCHG(const uint32_t pc) const
{
    ...
    {
        const uint8_t bit = (cdi.PeekWord(pc+2) & 0x00FF) % (eamode ? 8 : 32);
...
std::string SCC68070::DisassembleDBcc(const uint32_t pc) const
{
    const uint8_t condition = (currentOpcode & 0x0F00) >> 8;
    const uint8_t       reg = (currentOpcode & 0x0007);
    const int16_t      disp = cdi.PeekWord(pc+2);
...
{
    const uint8_t data = cdi.PeekWord(pc+2);
...
std::string SCC68070::DisassembleMOVEM(const uint32_t pc) const
{
    ...
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    const uint16_t mask = cdi.PeekWord(pc+2);
