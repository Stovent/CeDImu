...
{
    if(!m_sectorTime || !m_disc.IsOpen())
        return std::nullopt;

    *m_sectorTime += ns;
    if(*m_sectorTime < SECTOR_DELTA) // One sector every 1/75th second.
        return std::nullopt;
    *m_sectorTime -= SECTOR_DELTA;
 ...
 */
void CDDrive::StartPlaying(uint32_t startLsn, uint8_t file, uint32_t channelMask) noexcept
{
    m_sectorTime = 0.0;
    m_discLsn = startLsn;
    m_fileNumber = file;
...
/** \brief Stops the drive. */
void CDDrive::StopPlaying() noexcept
{
    m_sectorTime = std::nullopt;
}
