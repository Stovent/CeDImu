...
void SCC68070::DisassembleAbcd(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tABCD " + (rm ? "-(A" + std::to_string(Ry) + "), -(A" + std::to_string(Rx) + ")" : \
...
void SCC68070::DisassembleAdd(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tADD" + (size == 1 ? ".B " : size == 2 ? ".W " : ".L ") \
...
void SCC68070::DisassembleAdda(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tADDA" + (size == 2 ? ".W " : ".L ") + DisassembleAddressingMode(pc+2, eamode, eareg, size) + ", A" + std::to_string(reg));
}
...
void SCC68070::DisassembleAddi(uint32_t pc)
{
    ...
    {
        size = 1;
        data = std::to_string((int8_t)vdsc->GetByteNoDebug(pc+3));
    }
    else if(size == 1)
    {
        size = 2;
        data = std::to_string((int16_t)vdsc->GetWordNoDebug(pc+2));
    }
    else
    {
        size = 4;
        data = std::to_string((int32_t)vdsc->GetLongNoDebug(pc+2));
    }
...
void SCC68070::DisassembleAddq(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tADDQ" + (size == 1 ? ".B #" : size == 2 ? ".W #" : ".L #") + std::to_string(data) + ", " + DisassembleAddressingMode(pc+2, eamode, eareg, size));
}
...
void SCC68070::DisassembleAddx(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tADDX" + (size == 1 ? ".B " : size == 2 ? ".W " : ".L ") + (rm ? "-(A" + std::to_string(Ry) + "), -(A" + std::to_string(Rx) + ")" : \
                                                                                                                "D" + std::to_string(Ry) + ", D" + std::to_string(Rx)));
}
...
void SCC68070::DisassembleAnd(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tAND" + (size == 1 ? ".B " : size == 2 ? ".W " : ".L ") + (opmode < 3 ? DisassembleAddressingMode(pc+2, eamode, eareg, size) + ", D" + std::to_string(reg) : \
                                                                                                                       "D" + std::to_string(reg) + ", " + DisassembleAddressingMode(pc+2, eamode, eareg, size)));
}
...
void SCC68070::DisassembleAsM(uint32_t pc)
{
    ...
    const uint8_t  eareg = (currentOpcode & 0x0007);
    if(dr)
        instructionsBuffer.push_back(toHex(pc) + "\tASL " + DisassembleAddressingMode(pc+2, eamode, eareg, 2));
    else
        instructionsBuffer.push_back(toHex(pc) + "\tASR " + DisassembleAddressingMode(pc+2, eamode, eareg, 2));
}
...
void SCC68070::DisassembleAsR(uint32_t pc)
{
    ...
    if(dr)
        instructionsBuffer.push_back(toHex(pc) + "\tASL" + (size == 0 ? ".B " : size == 1 ? ".W " : ".L ") + leftOperand + ", D" + std::to_string(reg));
    else
        instructionsBuffer.push_back(toHex(pc) + "\tASR" + (size == 0 ? ".B " : size == 1 ? ".W " : ".L ") + leftOperand + ", D" + std::to_string(reg));
}
...
void SCC68070::DisassembleBchg(uint32_t pc)
{
    ...
    {
        const uint8_t bit = (vdsc->GetWordNoDebug(pc+2) & 0x00FF) % 8;
        data += "#" + std::to_string(bit) + ", " + DisassembleAddressingMode(pc+4, eamode, eareg, 4);
    }

    instructionsBuffer.push_back(toHex(pc) + "\tBCHG " + data);
}
...
void SCC68070::DisassembleBclr(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tBCLR " + data);
}
...
void SCC68070::DisassembleBset(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tBSET " + data);
}
...
void SCC68070::DisassembleBtst(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tBTST " + data);
}
...
void SCC68070::DisassembleClr(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tCLR" + (size == 1 ? ".B " : size == 2 ? ".W " : ".L ") + DisassembleAddressingMode(pc+2, eamode, eareg, size));
}
...
void SCC68070::DisassembleCmp(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tCMP" + (opmode == 1 ? ".B " : opmode == 2 ? ".W " : ".L ") + DisassembleAddressingMode(pc+2, eamode, eareg, opmode) + ", D" + std::to_string(reg));
}
...
void SCC68070::DisassembleCmpa(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tCMPA" + (opmode == 2 ? ".W " : ".L ") + DisassembleAddressingMode(pc+2, eamode, eareg, opmode) + ", A" + std::to_string(reg));
}
...
void SCC68070::DisassembleCmpm(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tCMPM" + (size == 1 ? ".B (A" : size == 2 ? ".W (A" : ".L (A") + std::to_string(Ay) + ")+, (A" + std::to_string(Ax) + ")+");
}
...
void SCC68070::DisassembleDbCC(uint32_t pc)
{
    const uint8_t condition = (currentOpcode & 0x0F00) >> 8;
    const uint8_t       reg = (currentOpcode & 0x0007);
    const int16_t      disp = vdsc->GetWordNoDebug(pc+2);
...
void SCC68070::DisassembleDivs(uint32_t pc)
{
    ...
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    instructionsBuffer.push_back(toHex(pc) + "\tDIVS.W " + DisassembleAddressingMode(pc+2, eamode, eareg, 2) + ", D" + std::to_string(reg));
}
...
void SCC68070::DisassembleDivu(uint32_t pc)
{
    ...
    const uint8_t  eareg = (currentOpcode & 0x0007);
    instructionsBuffer.push_back(toHex(pc) + "\tDIVU.W " + DisassembleAddressingMode(pc+2, eamode, eareg, 2) + ", D" + std::to_string(reg));
}
...
void SCC68070::DisassembleEor(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tEOR" + (opmode == 1 ? ".B D" : opmode == 2 ? ".W D" : ".L D") + std::to_string(reg) + ", " + DisassembleAddressingMode(pc+2, eamode, eareg, opmode));
}
...
void SCC68070::DisassembleEori(uint32_t pc)
{
    ...
    {
        size = 1;
        data = std::to_string(vdsc->GetByteNoDebug(pc+3));
    }
    else if(size == 1)
    {
        size = 2;
        data = std::to_string(vdsc->GetWordNoDebug(pc+2));
    }
    else
    {
        size = 4;
        data = std::to_string(vdsc->GetLongNoDebug(pc+2));
    }
...
{
    const uint8_t data = vdsc->GetWordNoDebug(pc+2);
...
{
    instructionsBuffer.push_back(toHex(pc) + "\tILLEGAL");
}
...
void SCC68070::DisassembleJmp(uint32_t pc)
{
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    instructionsBuffer.push_back(toHex(pc) + "\tJMP " + DisassembleAddressingMode(pc+2, eamode, eareg, 0));
}
...
void SCC68070::DisassembleJsr(uint32_t pc)
{
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    instructionsBuffer.push_back(toHex(pc) + "\tJSR " + DisassembleAddressingMode(pc+2, eamode, eareg, 0));
}
...
void SCC68070::DisassembleLsM(uint32_t pc)
{
    ...
    if(dr)
        instructionsBuffer.push_back(toHex(pc) + "\tLSL " + DisassembleAddressingMode(pc+2, eamode, eareg, 2));
    else
        instructionsBuffer.push_back(toHex(pc) + "\tLSR " + DisassembleAddressingMode(pc+2, eamode, eareg, 2));
}
...
void SCC68070::DisassembleLsR(uint32_t pc)
{
    ...
    if(dr)
        instructionsBuffer.push_back(toHex(pc) + "\tLSL" + (size == 0 ? ".B " : size == 1 ? ".W " : ".L ") + leftOperand + ", D" + std::to_string(reg));
    else
        instructionsBuffer.push_back(toHex(pc) + "\tLSR" + (size == 0 ? ".B " : size == 1 ? ".W " : ".L ") + leftOperand + ", D" + std::to_string(reg));
}
...
void SCC68070::DisassembleMove(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tMOVE." + ((size == 1) ? "B " : (size == 3) ? "W " : "L ") + srcEA + ", " + dstEA);
}
...
void SCC68070::DisassembleMovea(uint32_t pc)
{
    ...
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    instructionsBuffer.push_back(toHex(pc) + "\tMOVEA" + (size == 3 ? ".W " : ".L ") + DisassembleAddressingMode(pc+2, eamode, eareg, size == 3 ? 2 : 4, true) + ", A" + std::to_string(reg));
}
...
void SCC68070::DisassembleMoveccr(uint32_t pc)
{
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    instructionsBuffer.push_back(toHex(pc) + "\tMOVE " + DisassembleAddressingMode(pc+2, eamode, eareg, 2) + ", CCR");
}
...
void SCC68070::DisassembleMoveFsr(uint32_t pc)
{
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    instructionsBuffer.push_back(toHex(pc) + "\tMOVE SR, " + DisassembleAddressingMode(pc+2, eamode, eareg, 2));
}
...
void SCC68070::DisassembleMovesr(uint32_t pc)
{
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    instructionsBuffer.push_back(toHex(pc) + "\tMOVE " + DisassembleAddressingMode(pc+2, eamode, eareg, 2) + ", SR");
}
...
void SCC68070::DisassembleMovem(uint32_t pc)
{
    ...
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    const uint16_t mask = vdsc->GetWordNoDebug(pc+2);
...
void SCC68070::DisassembleMovep(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tMOVEP" + operands);
}
...
void SCC68070::DisassembleMoveq(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tMOVEQ #" + std::to_string(data) + ", D" + std::to_string(reg));
}
...
void SCC68070::DisassembleMuls(uint32_t pc)
{
    ...
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    instructionsBuffer.push_back(toHex(pc) + "\tMULS.W " + DisassembleAddressingMode(pc+2, eamode, eareg, 2) + ", D" + std::to_string(reg));
}
...
void SCC68070::DisassembleMulu(uint32_t pc)
{
    ...
    const uint8_t eamode = (currentOpcode & 0x0038) >> 3;
    const uint8_t  eareg = (currentOpcode & 0x0007);
    instructionsBuffer.push_back(toHex(pc) + "\tMULU.W " + DisassembleAddressingMode(pc+2, eamode, eareg, 2) + ", D" + std::to_string(reg));
}
...
void SCC68070::DisassembleNeg(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tNEG" + (size == 1 ? ".B " : size == 2 ? ".W " : ".L ") + DisassembleAddressingMode(pc+2, eamode, eareg, size));
}
...
void SCC68070::DisassembleNegx(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tNEGX" + (size == 1 ? ".B " : size == 2 ? ".W " : ".L ") + DisassembleAddressingMode(pc+2, eamode, eareg, size));
}
...
void SCC68070::DisassembleNot(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tNOT" + (size == 1 ? ".B " : size == 2 ? ".W " : ".L ") + DisassembleAddressingMode(pc+2, eamode, eareg, size));
}
...
void SCC68070::DisassembleOr(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tOR" + (size == 1 ? ".B " : size == 2 ? ".W " : ".L ") + (opmode < 3 ? DisassembleAddressingMode(pc+2, eamode, eareg, size) + ", D" + std::to_string(reg) : \
                                                                                                                      "D" + std::to_string(reg) + ", " + DisassembleAddressingMode(pc+2, eamode, eareg, size)));
}
...
{
    instructionsBuffer.push_back(toHex(pc) + "\tRESET");
}
...
void SCC68070::DisassembleRoM(uint32_t pc)
{
    ...
    if(dr)
        instructionsBuffer.push_back(toHex(pc) + "\tROL " + DisassembleAddressingMode(pc+2, eamode, eareg, 2));
    else
        instructionsBuffer.push_back(toHex(pc) + "\tROR " + DisassembleAddressingMode(pc+2, eamode, eareg, 2));
}
...
void SCC68070::DisassembleRoR(uint32_t pc)
{
    ...
    if(dr)
        instructionsBuffer.push_back(toHex(pc) + "\tROL" + (size == 0 ? ".B " : size == 1 ? ".W " : ".L ") + leftOperand + ", D" + std::to_string(reg));
    else
        instructionsBuffer.push_back(toHex(pc) + "\tROR" + (size == 0 ? ".B " : size == 1 ? ".W " : ".L ") + leftOperand + ", D" + std::to_string(reg));
}
...
void SCC68070::DisassembleRoxM(uint32_t pc)
{
    ...
    if(dr)
        instructionsBuffer.push_back(toHex(pc) + "\tROXL " + DisassembleAddressingMode(pc+2, eamode, eareg, 2));
    else
        instructionsBuffer.push_back(toHex(pc) + "\tROXR " + DisassembleAddressingMode(pc+2, eamode, eareg, 2));
}
...
void SCC68070::DisassembleRoxR(uint32_t pc)
{
    ...
    if(dr)
        instructionsBuffer.push_back(toHex(pc) + "\tROXL" + (size == 0 ? ".B " : size == 1 ? ".W " : ".L ") + leftOperand + ", D" + std::to_string(reg));
    else
        instructionsBuffer.push_back(toHex(pc) + "\tROXR" + (size == 0 ? ".B " : size == 1 ? ".W " : ".L ") + leftOperand + ", D" + std::to_string(reg));
}

void SCC68070::DisassembleRte(uint32_t pc)
{
    instructionsBuffer.push_back(toHex(pc) + "\tRTE");
}

void SCC68070::DisassembleRtr(uint32_t pc)
{
    instructionsBuffer.push_back(toHex(pc) + "\tRTR");
}

void SCC68070::DisassembleRts(uint32_t pc)
{
    instructionsBuffer.push_back(toHex(pc) + "\tRTS");
}
...
void SCC68070::DisassembleSbcd(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tSBCD " + (rm ? "-(A" + std::to_string(Rx) + "), -(A" + std::to_string(Ry) + ")" : \
...
void SCC68070::DisassembleSub(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tSUB" + (size == 1 ? ".B " : size == 2 ? ".W " : ".L ") + (opmode < 3 ? (DisassembleAddressingMode(pc+2, eamode, eareg, size) + ", D" + std::to_string(reg)) : \
                                                                                                                       ("D" + std::to_string(reg) + ", " + DisassembleAddressingMode(pc+2, eamode, eareg, size))));
}
...
void SCC68070::DisassembleSuba(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tSUBA" + (opmode == 2 ? ".W " : ".L ") + DisassembleAddressingMode(pc+2, eamode, eareg, opmode) + ", A" + std::to_string(reg));
}
...
void SCC68070::DisassembleSubq(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tSUBQ" + (size == 1 ? ".B #" : size == 2 ? ".W #" : ".L #") + std::to_string(data) + ", " + DisassembleAddressingMode(pc+2, eamode, eareg, size));
}
...
void SCC68070::DisassembleSubx(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tSUBX" + (size == 1 ? ".B " : size == 2 ? ".W " : ".L ") + (rm ? "-(A" + std::to_string(rx) + "), -(A" + std::to_string(ry) + ")" : \
                                                                                                                  "D" + std::to_string(rx) + ", D" + std::to_string(ry)));
}

void SCC68070::DisassembleSwap(uint32_t pc)
{
    instructionsBuffer.push_back(toHex(pc) + "\tSWAP D" + std::to_string(currentOpcode & 0x0007));
}
...
{
    instructionsBuffer.push_back(toHex(pc) + "\tTRAP #" + std::to_string(currentOpcode & 0x000F));
}

void SCC68070::DisassembleTrapv(uint32_t pc)
{
    instructionsBuffer.push_back(toHex(pc) + "\tTRAPV");
}
...
void SCC68070::DisassembleTst(uint32_t pc)
{
    ...
    instructionsBuffer.push_back(toHex(pc) + "\tTST " + DisassembleAddressingMode(pc+2, eamode, eareg, size));
}

void SCC68070::DisassembleUnlk(uint32_t pc)
{
    instructionsBuffer.push_back(toHex(pc) + "\tUNLK A" + std::to_string(currentOpcode & 0x0007));
}
