    ...
    , m_key12(Config::key12)
{
    ...
    // CDI page
    {
        wxPanel* cdiPage = new wxPanel(notebook);

        wxHyperlinkCtrl* helperLink = new wxHyperlinkCtrl(cdiPage, wxID_ANY, "See the MANUAL for more information", "https://github.com/Stovent/CeDImu/blob/master/MANUAL.md");

        // Disc path
        m_discPath = new wxTextCtrl(cdiPage, wxID_ANY, Config::discDirectory);
        wxButton* discSelect = new wxButton(cdiPage, wxID_ANY, "Select Discs directory", wxDefaultPosition, wxSize(125, 0));
        ...
        wxStaticBoxSizer* discSizer = new wxStaticBoxSizer(wxVERTICAL, cdiPage, "Disc");
        discSizer->Add(discPathRow, wxSizerFlags().Expand().Border());


        // BIOSes
        wxStaticText* helperTextReload = new wxStaticText(cdiPage, wxID_ANY, "After changing a BIOS configuration, click\n\"Emulation -> Reload core\" to apply the new configuration.");

        // BIOS Buttons
        wxButton* biosNewButton = new wxButton(cdiPage, IDSettingsFrameOnNewConfig, "New");
        wxButton* biosDeleteButton = new wxButton(cdiPage, IDSettingsFrameOnDeleteConfig, "Delete");
        ...
        m_biosList = new wxListBox(cdiPage, IDSettingsFrameOnSelectBios, wxDefaultPosition, wxDefaultSize, biosStrings);
        ...
        // BIOS config
        wxPanel* biosConfigPage = new wxPanel(cdiPage);
        ...
        wxStaticBoxSizer* boardSizer = new wxStaticBoxSizer(wxVERTICAL, cdiPage, "BIOS Configuration");
        boardSizer->Add(helperTextReload, wxSizerFlags().Border());
        boardSizer->Add(biosConfigPanelSizer, wxSizerFlags(1).Expand());

        wxBoxSizer* cdiSizer = new wxBoxSizer(wxVERTICAL);
        cdiSizer->Add(helperLink, wxSizerFlags().Border());
        cdiSizer->Add(discSizer, wxSizerFlags().Expand());
        cdiSizer->Add(boardSizer, wxSizerFlags().Expand());
        cdiPage->SetSizerAndFit(cdiSizer);

        notebook->AddPage(cdiPage, "CD-I");
    }
