...
DisassemblerFrame::DisassemblerFrame(SCC68070& core, MainFrame* parent, const wxPoint& pos, const wxSize& size) : wxFrame(parent, wxID_ANY, "SCC68070 Disassembler", pos, size), cpu(core)
{
    ...
    for(uint8_t i = 0; i < 8; i++)
    {
        ...
        a[i] = new wxTextCtrl(registersPanel, wxID_ANY, "A" + std::to_string(i) + ": " + std::to_string(cpu.A[i]), wxPoint(0, i * 23 + 189), wxSize(125, 23), wxTE_READONLY); a[i]->SetBackgroundColour(*wxWHITE);
    }
    pc = new wxTextCtrl(registersPanel, IDDisassemblerpc, "PC: " + std::to_string(cpu.PC), wxPoint(0, 377), wxSize(125, 23), wxTE_READONLY); pc->SetBackgroundColour(*wxWHITE);
...
void DisassemblerFrame::PaintEvent()
{
    if(cpu.instructionsBufferChanged)
    {
        for(uint8_t i = 0; i < 8; i++)
        {
            ...
            a[i]->SetLabelText("A" + std::to_string(i) + ": " + std::to_string(cpu.A[i]));
        }
        pc->SetLabelText("PC: " + std::to_string(cpu.PC));
